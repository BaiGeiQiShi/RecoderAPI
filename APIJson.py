...
def getJSON_API(pDir):

    ret=[]

    for p in (pathlib.Path(pDir)).glob('**/*.java'):
        r=parse_file(p)
        if r is not None:
            ret.append(r)
...
def parse_package1(pkg_name,ver_tag):
    pathlib.Path('var').mkdir(exist_ok=True)
...
def get_json(pkg):
    ...
    parse_package(pkg_name, pkg_id)

def get_json1(pkg):
    pkg_n, pkg_id = pkg.split('-')
    return parse_package1(pkg_n, pkg_id)
