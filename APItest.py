...
#    continue
#s = s[-1]
'''print("There are classes to modify, choose a index. \n--------------------------------------\n")
if len(s) != 1:
    nindex = 0
    for sub_elem in s:
        print(sub_elem)
        s[nindex] = s[nindex].replace("\n", "")
        nindex += 1
    print("\n----------------------------------\n")
    sindex = eval(input())
else:
...
locationdir = 'location/ochiai/%s/%s.txt' % (project, bugid)
if not os.path.exists(locationdir):
...
for loc in lines:
    lst = loc.strip().split(',')
    s, lineid= lst[0].split('#')    
    if '$' in s:
        cls = copy.deepcopy(s[:s.index('$')])
        s = cls;
    location.append((s, eval(lineid)))
...
pDir = '/105_bugs_with_src/%s%s' % (project, bugid)
pDir = cwd + pDir
dirs = os.popen('catena4j export -p dir.src.classes -w %s' % pDir).readlines()[0]
pDir = pDir + '/' + dirs 
...
wf.close()'''
data = []
#multiply repairing implelement in for loop
for j in range(40):
    if j >= len(location):
        break
    patchdict = {}
    ...
    lines1 = open(filepath, "r", encoding="latin-1").read().strip()
    ...
    if True:
        ...
        print("\n--------------------------\n Buggy line: \n"+oldcode+"\n---------------------------\n")
        data.append({'treeroot':treeroot, 'troot':troot, 'oldcode':oldcode, 'filepath':filepath, 'subroot':subroot, 'vardic':vardic, 'typedic':typedic, 'idss':'None', 'classname':classname, 'precode':precode, 'aftercode':aftercode, 'tree':troot.printTreeWithVar(troot, vardic), 'prob':troot.getTreeProb(troot), 'mode':0, 'line':lineid, 'isa':False, 'pDir':pDir, 'fDir':fDir})
...
solveone(data,model,project,bugid)
